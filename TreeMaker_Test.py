...
def make_tree(curr_grid, test_piece, depth, goatNum, curr_turn):
    Root = anytree.Node("root", grid=deepcopy(curr_grid), grid_matrix=grid_to_two_dimensions(curr_grid),
                        no_goats=deepcopy(goatNum))
    ChildNum = 0
                ...
                if (isBlack):
                    for Tiger in Tigers:
                        ...
                        for move in test_piece.possible_moves:
                            ...
                            if test_piece.update_grid(temp_grid, move):
                                anytree.Node("Child" + str(ChildNum), grid=deepcopy(temp_grid), move_done=move,
                                             no_goats=deepcopy(new_no_goats),
                                             turn=turn_counter,
                                             value=evaluate(temp_grid, deepcopy(test_piece), turn_counter,
                                                            new_no_goats), parent=node)
                                ChildNum += 1
                    ...
                    for Goat in Goats:
                        ...
                        for move in test_piece.possible_moves:
                            ...
                            if test_piece.update_grid(temp_grid, move):
                                anytree.Node("Child" + str(ChildNum), grid=deepcopy(temp_grid), move_done=move,
                                             no_goats=deepcopy(node.no_goats),
                                             turn=turn_counter,
                                             value=evaluate(temp_grid, deepcopy(test_piece), turn_counter,
                                                            node.no_goats), parent=node)
                                ChildNum += 1
        turn_counter += 1
